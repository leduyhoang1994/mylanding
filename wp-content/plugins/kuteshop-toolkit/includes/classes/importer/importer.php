<?php
/*
 * Plugin Name: Kuteshop Importer
 * Plugin URI: 
 * Description: Import Kuteshop theme demo data
 * Author: KuteTheme
 * Text Domain: kuteshop-toolkit-toolkit
...
if ( !class_exists( 'KUTESHOP_IMPORTER' ) ) {
	class KUTESHOP_IMPORTER
	{
		...
		function __construct()
		{
			...
			// SET DATA DEMOS
			$this->content_path   = plugin_dir_path( __FILE__ ) . 'data/content.xml';
			$this->widget_path    = plugins_url( '/data/widgets.wie', __FILE__ );
			$this->revslider_path = dirname( __FILE__ ) . '/data/revsliders/';
			$registed_menu        = array(
				'primary'         => esc_html__( 'Primary Menu', 'kuteshop-toolkit' ),
				'top_left_menu'   => esc_html__( 'Top Left Menu', 'kuteshop-toolkit' ),
				'top_right_menu'  => esc_html__( 'Top Right Menu', 'kuteshop-toolkit' ),
				'top_center_menu' => esc_html__( 'Top Center Menu', 'kuteshop-toolkit' ),
				'sticky_menu'     => esc_html__( 'Sticky Menu', 'kuteshop-toolkit' ),
				'vertical_menu'   => esc_html__( 'Vertical Menu', 'kuteshop-toolkit' ),
			);
			$menu_location        = array(
			...
			for ( $i = 1; $i <= 14; $i++ ) {
				$this->data_demos[] = array(
					'name'           => esc_html__( 'Demo ' . zeroise( $i, 2 ), 'kuteshop-toolkit-toolkit' ),
					'slug'           => 'home-' . zeroise( $i, 2 ),
					'menus'          => $registed_menu,
					'homepage'       => 'Home ' . zeroise( $i, 2 ),
					'blogpage'       => 'Blog',
					'preview'        => plugins_url( '/data/previews/' . $i . '.jpg', __FILE__ ),
					'demo_link'      => 'http://kuteshop.kutethemes.net/home-' . zeroise( $i, 2 ),
			...
			add_action( 'wp_ajax_kt_import_footer_content', array( $this, 'import_footer_content' ) );
			add_action( 'wp_ajax_kt_import_testimonial', array( $this, 'import_testimonial' ) );
			add_action( 'wp_ajax_kt_import_megamenu', array( $this, 'import_megamenu' ) );
			...
			$this->item_import = array(
				'kt_import_full_content'    => 'Import full content',
				'kt_import_page_content'    => 'Import Page',
				'kt_import_post_content'    => 'Import Post',
				'kt_import_megamenu'        => 'Import Mega Menu',
				'kt_import_testimonial'     => 'Import Testimonials',
				'kt_import_footer_content'  => 'Import Footer',
			...
			/* UPDTE IMAGE SIZE */
			$this->woo_catalog   = array(
				'width'  => '270',   // px
				'height' => '320',   // px
				'crop'   => 1        // true
			);
			$this->woo_single    = array(
				'width'  => '600',   // px
				'height' => '600',   // px
				'crop'   => 1        // true
			);
			$this->woo_thumbnail = array(
				'width'  => '180',   // px
				'height' => '180',   // px
				'crop'   => 1        // false
			);
		...
		{
			$this->define( 'KUTESHOP_IMPORTER_DIR', plugin_dir_path( __FILE__ ) );
			$this->define( 'KUTESHOP_IMPORTER_URI', plugin_dir_url( __FILE__ ) );
		}
		...
		function register_scripts()
		{
			...
			wp_enqueue_style( 'kt-importer-style', KUTESHOP_IMPORTER_URI . '/assets/circle.css' );
			wp_enqueue_style( 'kt-importer-circle', KUTESHOP_IMPORTER_URI . '/assets/import.css' );
			wp_enqueue_script( 'thickbox' );
			wp_enqueue_script( 'kt-importer-script', KUTESHOP_IMPORTER_URI . '/assets/import.js', array( 'jquery' ), false, true );
		}
		...
		function import_single_page_content()
		{
			$this->ajax_optionid         = isset( $_POST['optionid'] ) ? $_POST['optionid'] : '';
			$this->ajax_options_posttype = array( 'page' );
		...
		function import_footer_content()
		{
			$this->ajax_optionid         = isset( $_POST['optionid'] ) ? $_POST['optionid'] : '';
			$this->ajax_options_posttype = array( 'footer' );
			$this->import_content();
			wp_die();
		}

		function import_megamenu()
		{
			$this->ajax_optionid         = isset( $_POST['optionid'] ) ? $_POST['optionid'] : '';
			$this->ajax_options_posttype = array( 'megamenu' );
			$this->import_content();
			wp_die();
		}

		function import_testimonial()
		{
			$this->ajax_optionid         = isset( $_POST['optionid'] ) ? $_POST['optionid'] : '';
			$this->ajax_options_posttype = array( 'testimonial' );
			$this->import_content();
			wp_die();
		}
		...
		function import_content()
		{
			...
			if ( $ajax_optionid != '' ) {
				self::before_content_import();
				self::include_importer_classes();
		...
		function import_revslider()
		{
			...
			if ( class_exists( 'UniteFunctionsRev' ) && class_exists( 'ZipArchive' ) ) {
				...
				$updateAnim    = true;
				$updateStatic  = true;
				$rev_directory = $this->revslider_path;
				$rev_files     = array();
		...
		function import_config()
		{
			...
			self::menu_locations( $demo );
			self::update_options( $demo );
			wp_die();
		}
